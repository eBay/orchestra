...
import com.ebay.riskmgmt.workfloworchestrator.task.VizTask;
...
public class WorkflowVisulizationParser {
            ...
            switch (workflowMetadataType) {
                ...
                case branch:
                    if (workflowMetadata.getBranchWF() != null) {
                        ...
                        Workflow workflow = BranchFlow.Builder.aNewBranchFlow().named(workflowMetadata.getDesc().replaceAll("[+]","and").replaceAll("[,]","")).executeAsync(workflowMetadata.isAsync()).toExecute(toExecuteTask).addCases(map).build();
                        wfMap.put(name, workflow);
                    }
                    break;
            }
